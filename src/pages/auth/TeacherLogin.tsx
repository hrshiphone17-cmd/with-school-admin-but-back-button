    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="max-w-md w-full animate-slide-up">
        <div className="text-center mb-8">
          ...
          <h1 className="font-fredoka text-3xl font-bold">Teacher Login</h1>
          <p className="text-muted-foreground mt-2">Welcome back, educator!</p>
        </div>

        <form onSubmit={handleLogin} className="bg-card rounded-3xl p-8 shadow-playful space-y-6">
          <div className="space-y-2">
            <Label className="font-semibold text-base">School Email</Label>
            <Input
              type="email"
              placeholder="teacher@school.com"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="rounded-xl h-12 text-base border-2 focus:border-accent"
            />
          </div>
          <div className="space-y-2">
          ...
          <Button
            type="submit"
            disabled={loading}
            className="w-full rounded-2xl h-12 text-lg font-bold bg-accent text-accent-foreground shadow-playful hover:scale-[1.02] transition-transform"
          >
